...
for i in $(seq 1 $2)
	do
		...
		$mExec --enable-fs=1 -S0 -f $resultDir'/'$stem'/FOLD'$i'/FOLD'$i'.moses.log' -C5 -W1 -i $resultDir'/'$stem'/FOLD'$i'/FOLD'$i'.train' > $resultDir'/'$stem'/FOLD'$i'/FOLD'$i'.out'
		#Analyze out files
                java -jar addisAi.jar $resultDir'/'$stem'/FOLD'$i'/FOLD'$i
		#python evalCombo.py $resultDir'/'$stem'/FOLD'$i'/FOLD'$i $2
